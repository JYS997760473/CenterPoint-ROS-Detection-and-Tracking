...
#include "tracking/base_tracker_obsv_matcher.h"
...
class HmTrackingWorker : public BaseTrackingWorker {
 public:
  ...
  bool track(const std::vector<ObjectPtr>& objects_obsved, double timestamp, const TrackingOptions& options,
   ...
   */
  virtual inline const std::vector<ObjectTrackerPtr>& getTrackers() const { return multi_object_tracker_->getTrackers(); }

  virtual inline const std::vector<IdType>& getUnassignedTrackers() const {
    return multi_object_tracker_->getUnassignedTrackers();
  }

  virtual inline const std::vector<IdType>& getLostTrackers() const { return multi_object_tracker_->getLostTrackers(); }

  virtual inline const std::map<IdType, Trajectory>& collectTrajectories() const { return trajectory_poses_; }

  virtual inline const std::vector<ObjectPtr>& collectTrackingObjectsInWorld() const { return tracking_objects_; }
  ...
  virtual std::vector<FixedTrajectory>
  collectFixedTrajectories() { /******************************************************************************
                                * Copyright 2017 The Apollo Authors. All Rights Reserved.
                                *
                                * Licensed under the Apache License, Version 2.0 (the "License");
                                * you may not use this file except in compliance with the License.
...
#include "tracking/trackers/multi_object_tracker.hpp"
    ...
    class HmTrackingWorker : public BaseTrackingWorker {
     public:
      ...
      virtual std::vector<FixedTrajectory> collectFixedTrajectories() {
        ...
        for (; iter != trajectory_periods_.end();) {
          if (iter->second > 0.) {
            fixedTrajectories.push_back(std::make_tuple(iter->first, iter->second));
       ...
       * @return
       */
      virtual std::vector<ObjectPtr> collectExpectedObjects(const double& timestamp, const Eigen::Matrix4d& trans_pose) const {
     ...
     protected:
      ...
      bool bootloader(const std::vector<ObjectPtr>& objects_obsved, const double& timestamp, const TrackingOptions& options,
                      std::vector<ObjectPtr>* objects_tracked);
      ...
      void constructTrackedObjects(const std::vector<ObjectPtr>& objects_obsved,
                                   std::vector<TrackableObjectPtr>* objects_tracked, const TrackingOptions& options);
 ...
 protected:
  ...
  // @params[IN] options: tracker options with necessary information
  // @return nothing
  void constructTrackedObjects(const std::vector<ObjectPtr>& objects_obsved, std::vector<TrackableObjectPtr>* objects_tracked,
