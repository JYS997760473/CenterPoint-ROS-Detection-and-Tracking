...
#include "tracking/tracking_object.h"
...
class ObjectTracker {
 public:
  ...
  // maximum
  // @return true if set successfully, otherwise return false
  static bool setTrackerCachedHistorySizeMaximum(const int& tracker_cached_history_size_maximum);
  ...
  void getExpectedObject(const double& time_diff, TrackableObjectPtr expected_obj) const {
    ...
    expected_obj->anchor_point = current_object_->anchor_point + predicted_shift;
    expected_obj->barycenter = current_object_->barycenter + predicted_shift;
    expected_obj->ground_center = current_object_->ground_center + predicted_shift;
  ...
  // @return nothing
  void updateWithObservation(TrackableObjectPtr object_obsved, const double& time_diff);
  ...
  // @params[IN] time_diff: time interval from last updating
  // @return nothing
  void updateWithoutObservation(const Eigen::VectorXf& predict_state, const double& time_diff);
  //--------------------------- predict & update ---------------------------//
 ...
 protected:
  ...
  // @params[IN] time_diff: time interval between last two updating
  // @return true if track is static, otherwise return false
  bool checkTrackerStaticHypothesisByVelocityAngleChange(const double& time_diff);
  //------------------- smooth velocity and orientation --------------------//
