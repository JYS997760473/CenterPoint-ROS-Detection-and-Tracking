...
class BaseFilter {
 public:
  ...
  // @return nothing
  virtual void initState(const Eigen::Vector3f& anchor_point, const Eigen::Vector3f& velocity) = 0;
  ...
  // @return nothing
  virtual void updateWithObservation(TrackableObjectConstPtr new_object, TrackableObjectConstPtr old_object,
  ...
  // @return nothing
  virtual void getState(Eigen::Vector3f* anchor_point, Eigen::Vector3f* velocity, Eigen::Vector3f* velocity_accelaration) = 0;

  virtual void getAngularState(Eigen::Vector3f* orientation, Eigen::Vector3f* angular_velocity) = 0;
  ...
  virtual void getStateUncertainty(Eigen::Matrix3d* position_uncertainty, Eigen::Matrix3d* velocity_uncertainty) = 0;

  virtual float getVelocityUncertaintyLimit() const = 0;
