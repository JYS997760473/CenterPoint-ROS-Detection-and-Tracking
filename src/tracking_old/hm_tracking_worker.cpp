...
#include "tracking/hungarian_tracker_obsv_matcher.h"
#include "tracking/robust_kalman_filter.h"
#include "tracking/time.h"
#include "tracking/tracker_obsv_distance.h"
...
HmTrackingWorker::HmTrackingWorker(const TrackingWorkerParams& params)
  ...
  if (matcher_method_ == HUNGARIAN_MATCHER) {
    if (!HungarianTrackerObsvMatcher::setMatchDistanceMaximum(params_.matcher_match_distance_maximum)) {
      ROS_ERROR_STREAM("Failed to set match distance maximum! " << name());
      return;
    }
  }
  // load location distance weight
  if (!TrackerObsvDistance::setLocationDistanceWeight(params_.matcher_location_distance_weight)) {
    ROS_ERROR_STREAM("Failed to set location distance weight! " << name());
    return;
  }
  // load direction distance weight
  if (!TrackerObsvDistance::setDirectionDistanceWeight(params_.matcher_direction_distance_weight)) {
    ROS_ERROR_STREAM("Failed to set direction distance weight! " << name());
    return;
  }
  // load bbox size distance weight
  if (!TrackerObsvDistance::setBboxSizeDistanceWeight(params_.matcher_bbox_size_distance_weight)) {
    ROS_ERROR_STREAM("Failed to set bbox size distance weight! " << name());
    return;
  }
  // load point num distance weight
  if (!TrackerObsvDistance::setPointNumDistanceWeight(params_.matcher_point_num_distance_weight)) {
    ROS_ERROR_STREAM("Failed to set point num distance weight! " << name());
    return;
  }
  // load histogram distance weight
  if (!TrackerObsvDistance::setHistogramDistanceWeight(params_.matcher_histogram_distance_weight)) {
  ...
  if (filter_method_ == ROBUST_KALMAN_FILTER) {
    ...
    if (!RobustKalmanFilter::setAssociationScoreMaximum(params_.filter_association_score_maximum)) {
      ROS_ERROR_STREAM("Failed to set association score maximum! " << name());
      return;
    }
    /*
     * 初始化KF噪声
     ...
     */
    if (!RobustKalmanFilter::initNoiseParams(params_.filter_measurement_noise, params_.filter_initial_velocity_noise,
                                             params_.filter_xy_propagation_noise, params_.filter_z_propagation_noise)) {
      ROS_ERROR_STREAM("Failed to set params for kalman filter! " << name());
      return;
    }
    if (!RobustKalmanFilter::setBreakdownThresholdMaximum(params_.filter_breakdown_threshold_maximum)) {
      ROS_ERROR_STREAM("Failed to set breakdown threshold maximum! " << name());
      return;
    }
  }
  ...
  if (!ObjectTracker::setTrackerCachedHistorySizeMaximum(params_.tracker_cached_history_size_maximum)) {
    ROS_ERROR_STREAM("Failed to set tracking cached history size maximum! " << name());
    return;
  }
  // load acceleration maximum
  if (!ObjectTracker::setAccelerationNoiseMaximum(params_.tracker_acceleration_noise_maximum)) {
    ROS_ERROR_STREAM("Failed to set acceleration noise maximum! " << name());
    return;
  }
  // load speed noise maximum
  if (!ObjectTracker::setSpeedNoiseMaximum(params_.tracker_speed_noise_maximum)) {
    ROS_ERROR_STREAM("Failed to set speed noise maximum! " << name());
    return;
  }
  // load tracking consecutive invisible maximum
  if (!MultiObjectTracker::setTrackerConsecutiveInvisibleMaximum(params_.tracker_consecutive_invisible_maximum)) {
    ROS_ERROR_STREAM("Failed to set tracking consecutive invisible maximum! " << name());
    return;
  }
  // load tracking visible ratio minimum
  if (!MultiObjectTracker::setTrackerVisibleRatioMinimum(params_.tracker_visible_ratio_minimum)) {
    ROS_ERROR_STREAM("Failed to set track visible ratio minimum! " << name());
    return;
  }
  ...
  extractor_config.bin_size_for_histogram = static_cast<size_t>(params_.tracking_histogram_bin_size);
  feature_extractor_ = feature::createFeatureExtractor(extractor_config).get();
...
bool HmTrackingWorker::setMatcherMethod(const std::string& matcher_method_name) {
  if (matcher_method_name == "hungarian_matcher") {
    matcher_method_ = HUNGARIAN_MATCHER;
    ROS_INFO_STREAM("matcher method of " << name() << " is " << matcher_method_name);
    return true;
  }
...
bool HmTrackingWorker::setFilterMethod(const std::string& filter_method_name) {
  if (filter_method_name == "robust_kalman_filter") {
    ...
    ROS_INFO_STREAM("filter method of " << name() << " is " << filter_method_name);
    return true;
  }
 ...
 */
bool HmTrackingWorker::track(const std::vector<ObjectPtr>& objects_obsved, double timestamp, const TrackingOptions& options,
                             std::vector<ObjectPtr>* objects_tracked) {
  Clock clock;
  ROS_WARN_STREAM("HmTrackingWorker::track with " << objects_obsved.size() << " observed objects.");
  ...
  std::vector<TrackableObjectPtr> objects_trackable;
  constructTrackedObjects(objects_obsved, &objects_trackable, options);
  ...
  std::vector<ObjectTrackerPtr>& trackers = multi_object_tracker_->getTrackers();
  matcher_->match(&objects_trackable_transformed, trackers, tracker_predicts, &assignments, &unassigned_trackers,
                  &unassigned_obsvs);
  ROS_INFO_STREAM("multi-object-tracking: "
  ...
  multi_object_tracker_->updateAssignedTrackers(&objects_trackable_transformed, assignments, time_diff);
  ...
  for (size_t i = 0u; i < assignments.size(); ++i) {
    ...
    if (iter != trajectory_segments_.end()) {
      iter->second.push_back(objects_trackable_transformed[obsv_id]->object_ptr->id);
    }
  }
  // E.2 update tracks without associated objects
  multi_object_tracker_->updateUnassignedTrackers(tracker_predicts, unassigned_trackers, time_diff);
  // E.3 Delete lost trackers in system tracking
  // multi_object_tracker_->removeLostTrackers();
  multi_object_tracker_->removeLostTrackers(&trajectory_poses_, &trajectory_periods_);
  ...
  // unassigned_obsvs);
  multi_object_tracker_->createNewTrackers(objects_trackable_transformed, unassigned_obsvs, &trajectory_segments_,
                                           &trajectory_periods_);
 ...
 * @return
 */
bool HmTrackingWorker::bootloader(const std::vector<ObjectPtr>& objects_obsved, const double& timestamp,
                                  const TrackingOptions& options, std::vector<ObjectPtr>* objects_tracked) {
  /**
   * In this project, we just track the objects in lidar_top
   */
  Clock clock;
  ...
  std::vector<int> unassigned_obsvs;
  unassigned_obsvs.resize(objects_trackable.size());
  std::iota(unassigned_obsvs.begin(), unassigned_obsvs.end(), 0);
  multi_object_tracker_->createNewTrackers(objects_trackable, unassigned_obsvs);
  // multi_object_tracker_->createNewTrackers(objects_trackable, unassigned_obsvs, &trajectory_segments_,
  // &trajectory_periods_);
  time_stamp_ = timestamp;
  ...
  ROS_INFO_STREAM("HmTrackingWorker::bootloader. Took " << clock.takeRealTime() << "ms.");
 ...
 * @return nothing
 */
void HmTrackingWorker::constructTrackedObjects(const std::vector<ObjectPtr>& objects_obsved,
  ...
  for (size_t i = 0u; i < num_obsvs; ++i) {
    ...
    (*objects_tracked)[i]->anchor_point = (*objects_tracked)[i]->anchor_point;
  }

  ROS_INFO_STREAM("HmTrackingWorker::constructTrackedObjects. Took " << clock.takeRealTime() << "ms.");
}
 ...
 */
void HmTrackingWorker::collectTrackingObjects(std::vector<ObjectPtr>* objects_tracked) {
  Clock clock;
  ...
  for (size_t i = 0u; i < trackers.size(); ++i) {
    // <1>不输出追踪丢失的障碍物
    if (trackers[i]->consecutive_invisible_count_ > params_.tracking_collect_consecutive_invisible_maximum) {
      continue;
    }
    // <2>追踪器存活帧数需要满足要求
    ...
    obj->yaw_rad = common::geometry::computeTheta2dXyBetweenVectors<Eigen::Vector3d>(coord_dir, obj->direction);
    ...
    // // direction
    // common::transform::transformDirection(pose_world2velo, &(obj->direction));
    // obj->yaw_rad = common::geometry::computeTheta2dXyBetweenVectors<Eigen::Vector3d>(coord_dir, obj->direction);
    // // ground center
    // common::transform::transformGroundBox(pose_world2velo, &(obj->ground_center));
    // // velocity
  ...
  ROS_INFO_STREAM("HmTrackingWorker::collectTrackingObjects. Took " << clock.takeRealTime() << "ms.");
}
